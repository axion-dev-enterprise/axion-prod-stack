...
FLOW_BUDGET_SCAN_ORIGIN_VALUE="$(read_env_value FLOW_BUDGET_SCAN_ORIGIN)"
...
export \
  ...
  FLOW_BUDGET_SERVER_URL="$(read_env_value FLOW_BUDGET_SERVER_URL)" \
  FLOW_BUDGET_SERVER_METHOD="$(read_env_value FLOW_BUDGET_SERVER_METHOD)" \
  FLOW_BUDGET_SERVER_TOKEN="$(read_env_value FLOW_BUDGET_SERVER_TOKEN)" \
  FLOW_BUDGET_SERVER_TOKEN_HEADER="$(read_env_value FLOW_BUDGET_SERVER_TOKEN_HEADER)" \
  FLOW_BUDGET_SERVER_HOST_HEADER="$(read_env_value FLOW_BUDGET_SERVER_HOST_HEADER)" \
  FLOW_BUDGET_SERVER_DOWNLOAD_FIELD="$(read_env_value FLOW_BUDGET_SERVER_DOWNLOAD_FIELD)" \
  FLOW_BUDGET_SERVER_FALLBACK_FIELDS="$(read_env_value FLOW_BUDGET_SERVER_FALLBACK_FIELDS)" \
  FLOW_BUDGET_ASSISTANT_NAME="$(read_env_value FLOW_BUDGET_ASSISTANT_NAME)" \
...
python3 "$ROOT_DIR/scripts/tenants/render_flow_budget_agent.py" \
  "$TENANT_SLUG" \
  "$TENANT_HOST" \
  "$DATA_DIR" \
  "${FLOW_BUDGET_SCAN_ORIGIN_VALUE:-https://scan.axionenterprise.cloud}"
